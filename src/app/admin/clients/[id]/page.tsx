...
export default function ClientDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <>
      ...
      <div className="p-8">
        <Link href="/admin/clients" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-slate-700 mb-6">
            ...
            <div className="card">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">{client.company || 'Particulier'}</p>
                </div>
              </div>
              ...
              {editMode ? (
                ...
                <div className="space-y-3">
                  ...
                  {client.company && (
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Building2 className="w-4 h-4 text-slate-400" /> {client.company}
                    </div>
                  )}
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-100">
                    <button onClick={() => setEditMode(true)} className="btn-secondary text-xs flex-1 justify-center">Modifier</button>
                    <button onClick={toggleActive} className={`text-xs flex items-center gap-1 px-3 py-2 rounded-xl ${client.is_active ? 'text-accent-green' : 'text-slate-400'}`}>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-display font-bold flex items-center gap-2">
                  <FolderKanban className="w-5 h-5 text-purple" /> Projets ({projects.length})
                </h3>
                <Link href={`/admin/projects/new?client=${client.id}`} className="text-xs text-purple hover:underline">+ Ajouter</Link>
              </div>
              {projects.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {projects.map((p) => (
                    <Link key={p.id} href={`/admin/projects/${p.id}`} className="flex items-center gap-3 py-3 hover:bg-slate-50 px-2 rounded-lg transition-colors">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{p.name}</p>
                        <p className="text-xs text-slate-400">{p.domain || 'Pas de domaine'}</p>
                      </div>
            ...
            <div className="card">
              <h3 className="font-display font-bold flex items-center gap-2 mb-4">
                <CreditCard className="w-5 h-5 text-accent-green" /> Abonnements ({subscriptions.length})
              </h3>
              {subscriptions.length === 0 ? (
                <p className="text-sm text-slate-400 text-center py-4">Aucun abonnement</p>
              ) : (
                <div className="divide-y divide-slate-100">
                  {subscriptions.map((s) => (
                    <div key={s.id} className="flex items-center gap-3 py-3">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-400">{(s.project as any)?.name || 'Général'}</p>
                      </div>
            ...
            <div className="card">
              <h3 className="font-display font-bold flex items-center gap-2 mb-4">
                <LifeBuoy className="w-5 h-5 text-coral" /> Tickets récents ({tickets.length})
              </h3>
              {tickets.length === 0 ? (
