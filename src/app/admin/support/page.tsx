          ...
          {['all', 'open', 'closed'].map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                filter === f ? 'bg-purple text-white' : 'bg-white text-slate-500 border border-slate-200 hover:bg-slate-50'
              }`}
            >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left text-xs font-semibold text-slate-500 p-4">Ticket</th>
                ...
                <th className="text-left text-xs font-semibold text-slate-500 p-4">Priorité</th>
                ...
                <th className="text-left text-xs font-semibold text-slate-500 p-4">Actions</th>
              </tr>
            </thead>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">{(ticket.client as any)?.company}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 text-sm text-slate-600">{(ticket as any).project?.name || '—'}</td>
                  <td className="p-4"><StatusBadge status={ticket.priority} /></td>
                  <td className="p-4"><StatusBadge status={ticket.status} /></td>
                  <td className="p-4 text-xs text-slate-400">{formatDateTime(ticket.created_at)}</td>
                  <td className="p-4">
                    <select
